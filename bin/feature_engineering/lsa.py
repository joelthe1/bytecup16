...
# first 3 for questions and next 3 for user features
def run(ncomp=[500, 500, 20, 700, 700, 20]):
    ...
    tsvd = TruncatedSVD(n_components=ncomp[0], n_iter=7, random_state=42)
    ...
    tsvd = TruncatedSVD(n_components=ncomp[1], n_iter=7, random_state=42)
    ...
    tsvd = TruncatedSVD(n_components=ncomp[2], n_iter=7, random_state=42)
...
#    question_feature_map.clear()
    ...
    tf_word = TfidfTransformer(use_idf=True, norm='l2').fit(word_counts)
    word_tfs = tf_word.transform(word_counts)
    tsvd = TruncatedSVD(n_components=ncomp[3], n_iter=7, random_state=42)
    word_tfs = tsvd.fit_transform(word_tfs)
    ...
    tf_char = TfidfTransformer(use_idf=True, norm='l2').fit(char_counts)
    char_tfs = tf_char.transform(char_counts)
    tsvd = TruncatedSVD(n_components=ncomp[4], n_iter=7, random_state=42)
    char_tfs = tsvd.fit_transform(char_tfs)
    ...
    tf_tags = TfidfTransformer(use_idf=True, norm='l2').fit(tag_counts)
    tag_tfs = tf_tags.transform(tag_counts)
    tsvd = TruncatedSVD(n_components=ncomp[5], n_iter=7, random_state=42)
    tag_tfs = tsvd.fit_transform(tag_tfs)
...
#    user_clusters2 = sclustering.labels_
    ...
    print len(question_feature_map), len(user_feature_map)
