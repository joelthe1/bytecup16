...
for train_index, test_index in skf.split(X, y):
    print 'Cross-Validation run:{}'.format(ci+1)
    ci+=1
    ...
    for i,XV in enumerate(Xtrain):
        train_matrix[u_index_map[XV[0]]][q_index_map[XV[1]]] = int(ytrain[i])
    ...
    print model.user_embeddings.shape, sparse.csr_matrix(user_features).shape
...
print 'mean AUC on test: {}'.format(np.mean(auc))
